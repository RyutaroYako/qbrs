...
macro_rules! sql_leaf_type {
    ($name:ident, $native:ty, $null_variant:ident) => {
        ...
        // A plain value on the right of a nullable column's comparison.
        // `Option` deliberately has no such impl: `x = NULL` is never true,
        // so `.eq(None)` would be a predicate that cannot match, and
        // `.is_null()` is the question that was meant.
        impl IntoExpr<crate::scope::Nullable<$name>> for $native {
            type Req = Nil;
            fn into_expr(self) -> Expr<Nil, crate::scope::Nullable<$name>> {
                Expr::from_kind(ExprKind::Value(Value::from(self)))
            }
        }
...
impl IntoExpr<crate::scope::Nullable<Text>> for &str {
    type Req = Nil;
    fn into_expr(self) -> Expr<Nil, crate::scope::Nullable<Text>> {
        Expr::from_kind(ExprKind::Value(Value::Text(self.to_string())))
    }
}
