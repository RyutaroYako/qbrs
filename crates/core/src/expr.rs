...
pub enum ExprKind {
    ...
    Raw(Fragment),
    /// `CAST(expr AS type)`. Standard syntax rather than Postgres's `::`,
    /// so the same node renders on every dialect.
    Cast {
        expr: Box<ExprKind>,
        sql_type: &'static str,
    },
...
pub trait Summable: SqlType {
    type Sum: SqlType;
    const SUM_CAST: Option<&'static str>;
    const AVG_CAST: Option<&'static str> = Some("DOUBLE PRECISION");
}
impl Summable for Integer {
    type Sum = crate::scope::Nullable<BigInt>;
    const SUM_CAST: Option<&'static str> = None;
}
impl Summable for BigInt {
    type Sum = crate::scope::Nullable<BigInt>;
    const SUM_CAST: Option<&'static str> = Some("BIGINT");
}
...
impl<T: Summable> Summable for crate::scope::Nullable<T> {
    type Sum = T::Sum;
    const SUM_CAST: Option<&'static str> = T::SUM_CAST;
    const AVG_CAST: Option<&'static str> = T::AVG_CAST;
}
...
fn aggregate_kind<C: ColumnKey>(name: &'static str, cast: Option<&'static str>) -> ExprKind {
