...
| [`11_upsert`](examples/11_upsert.rs) | `ON CONFLICT (..) DO NOTHING` / `DO UPDATE SET ..`, gated to dialects with `SupportsOnConflict`, `partial_index(..)` for a partial unique index, and `excluded(..)` for the row the insert proposed |
